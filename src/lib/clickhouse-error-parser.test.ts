...
WHERE 
    ...
    const detailMessage = "Code: 47. DB::Exception: Unknown expression identifier `compressed_size` in scope SELECT count(1) AS part_count, sum(rows) AS rows, sum(bytes_on_disk) AS disk_size, sum(data_uncompressed_bytes) AS uncompressed_size, round(sum(data_uncompressed_bytes) / sum(data_compressed_bytes), 2) AS compress_ratio, round(disk_size / rows, 2) AS avg_row_size FROM system.parts WHERE (database = 'bithon') AND (`table` = 'bithon_trace_span_summary_local_v4') AND (active = 1) ORDER BY compressed_size DESC";
    const errorCode = "47";
    ...
    expect(location?.contextLines.some(l => l.isErrorLine && l.content.includes("ORDER BY compressed_size"))).toBe(true);
  ...
  it("should parse error code 47 with 'Missing columns' pattern", () => {
    ...
    const detailMessage = "Code: 47. DB::Exception: Missing columns: 'col_not_exist' while processing query";
    const errorCode = "47";
