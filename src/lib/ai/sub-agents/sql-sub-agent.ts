...
import { tools as clientTools } from '../client-tools';
...
2. Don't wrap the JSON in any additional text or formatting.`;
    ...
    try {
        ...
        const sanitizedHistory: any[] = history && history.length > 0
            ? history.map((msg, i) => {
                ...
                if (msg.role === 'function' || msg.role === 'tool') {
                    ...
                    const output = typeof resultValue === 'string' 
                ...
                if (msg.role === 'assistant' && (msg.content?.includes('Performing:') || msg.content?.includes('Action:'))) {
        ...
        for (let i = 0; i < sanitizedHistory.length - 1; i++) {
            if (sanitizedHistory[i].role === 'assistant' && sanitizedHistory[i + 1].role === 'tool') {
                ...
                if (typeof sanitizedHistory[i].content === 'string') {
                    ...
                    const exists = sanitizedHistory[i].content.some((p: any) => 
                        p.type === 'tool-call' && p.toolCallId === toolResult.toolCallId
                    );
                    if (!exists) {
        ...
        if (processResult.toolCalls && processResult.toolCalls.length > 0) {
            console.log('🔍 SQL Sub-Agent requested validation:', processResult.toolCalls);
        ...
        console.log('✅ SQL sub-agent output received and validated:', validated);
        return validated;
