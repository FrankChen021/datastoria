...
export function createCodeSearchTools(args: { provider: CodeSearch; maxSearchResults: number }) {
  ...
  return {
    search_file: tool({
      description:
        "Search the configured source project for relevant lines before reading files. Use this first to discover candidate files and line numbers. Query is plain text and matches case-insensitively.",
    ...
    read_file: tool({
      description:
        "Read a targeted section of a file from the configured source project. Prefer narrow line ranges after using `search_file`.",
      inputSchema: z.object({
