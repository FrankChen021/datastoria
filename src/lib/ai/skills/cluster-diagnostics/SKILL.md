...
1. Determine if the user asks about current state or a time-windowed status.
2. For current state, call `collect_cluster_status` first. Never provide current-health conclusions without this call.
3. For time-window questions (for example "past 3 hours"), call `collect_cluster_status` with `status_analysis_mode="windowed"`.
