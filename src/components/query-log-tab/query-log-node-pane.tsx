...
import type { NodeDetails } from "./query-log-graph";
...
function EdgeTable({ edges, type, emptyMessage = "No query available" }: EdgeTableProps) {
  ...
  const commaNumberFormatter = useMemo(() => Formatter.getInstance().getFormatter("comma_number"), []);
        ...
        <tbody className="[&_tr:last-child]:border-0">
          {sortedData.map((row, index) => (
            <tr key={index} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
              ...
              <td className="p-4 align-middle !p-2 whitespace-nowrap">
                {row.type === "QueryStart" ? "Started" : row.type === "QueryFinish" ? "Finished" : "Exception"}
              </td>
...
interface QueryLogNodePaneProps {
  selectedNode: NodeDetails;
  onClose: () => void;
}

export const QueryLogNodePane = memo(function QueryLogNodePane({
  selectedNode,
  onClose,
}: QueryLogNodePaneProps) {
  ...
  return (
    <Panel defaultSize={40} minSize={5} maxSize={70} className="bg-background border-l shadow-lg flex flex-col">
      ...
      <div className="flex-1 overflow-auto space-y-2 pb-16">
        ...
        <CollapsibleSection title="Incoming Queries" className="border-0 rounded-none" defaultOpen={true}>
        ...
        <CollapsibleSection title="Outgoing Queries" className="border-0 rounded-none" defaultOpen={true}>
          <div className="px-3 py-1">
            <EdgeTable
