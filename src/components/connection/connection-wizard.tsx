...
export function ConnectionWizard({
  ...
  const cardClassName = isDialog ? "w-full border-0 bg-transparent shadow-none" : "w-full";
  const editCardClassName = `${cardClassName} relative flex flex-col max-h-[calc(100dvh-2rem)] min-h-0`;
  ...
  const content = (
    <div
      className={
        isDialog ? "w-full max-w-2xl flex flex-col" : "w-full max-w-2xl flex flex-col my-auto"
      }
    >
  ...
  if (isDialog) {
    return <div className="w-full max-h-[calc(100dvh-2rem)] overflow-y-auto">{content}</div>;
  }
