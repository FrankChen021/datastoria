...
# Example for Google OAuth (provider is enabled when both credentials are set):
...
- **Login page shows no buttons**: Ensure `CLIENT_ID` and `CLIENT_SECRET` (and `TENANT_ID` for Microsoft) are set for your provider(s).
